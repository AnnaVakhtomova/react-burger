...
import { TErrorReponse } from "../../api-types";
import { Error403 } from "../../api";
...
export function login(email: string, password: string): AppThunk<void> {
  return function (dispatch) {
    dispatch({
      type: LOGIN_REQUEST,
    });
...
function getTokenFromResponse(accessToken: string): string {
...
export function getUserInfo(): AppThunk<void> {
...
export const withAccessToken = async <T>(
  ...
  try {
    ...
    if (err && typeof err === "string" && err === "403") {
      try {
...
export function checkAuth(): AppThunk<void> {
...
async function updateToken(
  ...
  const tokenlocal: string | null = localStorage.getItem("refreshToken");
  if (!tokenlocal) {
    Promise.reject();
  }
