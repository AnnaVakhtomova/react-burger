...
export function addIngredient(id: string): AppThunk<void> {
  return function (dispatch, getState) {
    ...
    if (ingredient.type === "bun") {
      ...
      const uuid = uuidv4();
      dispatch({ type: ADD_INGREDIENT, ingredient: { ...ingredient, uuidv4 } });
    }
...
export function deleteIngredient(index: number): AppThunk<void> {
  return function (dispatch, getState) {
    const state = getState();
